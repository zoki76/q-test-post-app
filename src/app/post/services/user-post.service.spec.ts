...
import { ENV_CONFIG } from '../../core/http/interfaces/environment-config.interface';
...
describe('UserPostService', () => {
  ...
  it('should return post list results', () => {
    apiServiceSpy.get.and.returnValues(of(posts), of(users), of(comments));
    spyOn<any>(service, 'findUser').and.callThrough();
    spyOn<any>(service, 'getPostListItems').and.callThrough();
    service.resolve().subscribe((data) => {
      expect(service['getPostListItems']).toHaveBeenCalledWith(
        posts,
        users,
        comments
      );
